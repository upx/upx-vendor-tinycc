...
[[ -d $tmpdir ]] || exit 1
cp -ai COPYING *.c *.h include lib tests win32 $tmpdir
...
rm lib/Makefile tests/Makefile tests/pp/Makefile tests/tests2/Makefile
# 5) cleanup files (whitespace)
sed -i -e 's/\r//' tests/tcctest.c
find . -type d -name '.git' -prune -o -type f -print0 | xargs -0r sed -i -e 's/[ \t\x0c]*$//'
expand_files=( COPYING tests/arm-asm-testsuite.sh win32/tcc-win32.txt )
for f in $expand_files **/*.[chS]; do expand -t 8 $f > $f.tmp; mv $f.tmp $f; done
